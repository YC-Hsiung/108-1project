...
function caculate_peaks()
    ...
    if size(shift_data,1)<1 | mod(peaks_had_caculated,20)~=0
        return
    end
    ...
    for i=0:3
        ...
        time_x=time_x*4;
        errorbar(time_x,shift_data(:,9*i+7),shift_data(:,9*i+8),char(line_format(i+1)),'DisplayName',['channel' char(i+49)]);
    end
